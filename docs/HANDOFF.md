...
>
> **Addendum (2026-05-20 realtime recovery).** Current pushed `master` is `f516ee4` or newer after the realtime-control recovery batch. The important commits are `3b97f5c` (promote pending chain during mid-fade updates so fast sweeps cannot pin the output thread in permanent 2x DSP), `457e244` + `cf98360` (single-in-flight LUFS preview worker, track-epoch safety, stale pending/cache cleanup), `e823f23` (rAF-gated latest-wins `updateChain` dispatch), `53d8c74` (diagnostic live-update counters), and `f516ee4` (Tone Shape knob colors now match the Visual EQ nodes they control: Low cyan, Mid purple, High blue). Dan observed prior fast-knob stutter and LUFS/meter confusion before this batch; next session should manually retest aggressive knob/EQ/Advanced sweeps with audio playing before starting new feature work.
>
...
>
> **Most recent verification note:** The realtime recovery commits after `5913ea7` have been reviewed/spot-checked in code, but this handoff addendum did not rerun the full suite. Last recorded command outputs remain in `docs/progress.md`; run at least `npm test`, `npm run build`, and `cargo test --lib` after pulling on the next machine if using this as a fresh baseline.
>
