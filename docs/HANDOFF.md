...
> **Current snapshot (2026-05-14 evening, post-audit).** App renamed Album Mastering Studio → **YES Master**. All seven UI restyle slices + the post-restyle UX restructure shipped earlier in the day; Codex then shipped four passes that stuck (`a3fcc25` console layout + real-PCM Original-playback metering, `6a441d9` rename, `1ea2fa5` 1920×1080 logical canvas bump, `4878140` force 100% webview zoom on launch). Codex also tried a Windows-DPI runtime workaround in `a09fe28` (force physical window to 1920×1080, `set_zoom(1/scale_factor)`) and **reverted it one minute later** at `fc7fa9f`. Treat that workaround as rejected for now, not an open fix to re-land: the product target is a 1920×1080 logical app at 100% webview zoom, and Dan's ~2880×1620 screenshots are expected Windows 150% display-scaling behavior. Do not shrink the app or counteract OS scaling unless Dan explicitly reopens the decision. Full-suite test status is stale after later UI/window/zoom work; last fully recorded green was at `6a441d9` (`cargo test --lib` 80/80, `cargo test` 138/138 fast lane, `npm run build` clean). **Rerun both as a first move next session.**
>
